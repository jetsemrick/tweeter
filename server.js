  ...
  WHERE username='${req.session.uid}'`,
    function(err, result, fields) {
      ...
      if (!bcrypt.compareSync(req.body.password, result[0].password))
        res.send(
          "Incorrect password. Please <a href='/register'>go back</a> and try again."
        );
    ...
    connection.query(
      ...
      function(err, result, fields) {
        ...
        if (result.length > 0) {
          ...
          res.send(
            "Username already in use. Please <a href='/edit/profile'>go back</a> and try something different."
          );
        } else if (String(req.body.username).length > 12) {
    ...
    connection.query(
      ...
      function(err, result, fields) {
        ...
        if (result.length > 0) {
          ...
          res.send(
            "Email already in use. Please <a href='/edit/profile'>go back</a> and try something different."
          );
        } else if (String(req.body.email).length > 100) {
